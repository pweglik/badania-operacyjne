...
\begin{enumerate}
  \item \lstinline{rotation_to_right} - losuje spójny ciąg przystanków w linii i przesuwa je o zadaną (lub losową) liczbę pozycji
    ...
    \item \lstinline{swap_every_nth} - losuje spójny ciąg przystanków w linii, i zamienia miejscami sąsiadów,
    \item \lstinline{swap_random} - losuje dwa różne przystanki, i zamienia je miejscami,
\end{enumerate}
...
\begin{enumerate}
  	...
  	\item \lstinline{line_based_merge(G1, G2)} - dzieli każdą z linii z G1 i G2 na połowy i jedną z połów każdej linii łączy z połową lini z drugiego genotypu. Z 4 możliwych przypadków połączenia wybiera ten, w którym dystans pomiędzy połączonymi przystankami jest minimalny
\end{enumerate}
